...
//* Заглушка: пока проверяем лишь, собрал ли игрок все 8 фрагментов.
 
router.get('/final/:tg_id', authenticate, async (req, res) => {
  try {
    const tg_id = req.params.tg_id

    // 1) Получаем массив fragments из БД
    ...
    // Возвращаем canEnter только на основании наличия всех 8 фрагментов
...
/* ── Полноценная версия со временем ──────────────────────────────────────
router.get('/final/:tg_id', authenticate, async (req, res) => {
  try {
    const tg_id = req.params.tg_id
    const { rows: [player] } = await pool.query(
    ...
    const gotAll    = fragments.length === 8

    // Проверка «окна» на 60 секунд от момента создания
    const createdMs = new Date(player.created_at).getTime()
    const delta     = Math.abs(Date.now() - createdMs)
    const inWindow  = delta <= 60_000  // 1 минута

    return res.json({
      canEnter: gotAll && inWindow,
      gotAll,
      inWindow,
      msLeft: inWindow ? 60_000 - delta : 0
    })
  } catch (err) {
    console.error('Error in GET /api/final/:tg_id', err)
    return res.status(500).json({ error: 'internal' })
