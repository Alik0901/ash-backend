...
// ── Helpers ─────────────────────────────────────────────────
function sign(user) {
  return jwt.sign({ tg_id: user.tg_id, name: user.name }, JWT_SECRET, {
    expiresIn: '1h'
  });
}
function randRef() {
...
// POST /api/init — регистрация + подтверждение рефералки
      ...
      try {
        ...
        const { rows: [me] } = await client.query(
          `INSERT INTO players
           ...
           RETURNING *`, [tg_id, name.trim()||null, code]
        );
        if (referrer_code) {
...
// GET /api/player/:tg_id — публичный профиль + число confirmed referrals
  ...
  try {
    ...
    const { rows: [c] } = await pool.query(
      `SELECT COUNT(*) AS cnt FROM referrals
         WHERE referrer_id=$1 AND status='confirmed'`, [req.params.tg_id]
    );
...
router.get('/stats/total_users', async (_req, res) => {
  try {
    const { rows: [r] } = await pool.query(`SELECT COUNT(*) AS cnt FROM players`);
...
router.get('/burn-status/:invoiceId', async (req, res) => {
  try {
    ...
    if (inv.status !== 'paid') {
      // не бросаем ошибку — front увидит {paid:false}
...
router.delete('/player/:tg_id', async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `DELETE FROM burn_invoices WHERE tg_id=$1`, [req.user.tg_id]
    );
    await client.query(
      `DELETE FROM players WHERE tg_id=$1`, [req.user.tg_id]
    );
    await client.query('COMMIT');
